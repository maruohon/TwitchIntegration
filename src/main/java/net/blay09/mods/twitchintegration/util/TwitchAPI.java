...
public class TwitchAPI {
    ...
    public static boolean listenForToken(final Runnable callback) {
        ...
        try {
            ...
            LiteModTwitchIntegration.logger.error("Could not open your browser - please copy the link into your browser manually: {}", uri);
            return false;
        }
    }
    ...
    public static void requestUsername(final String token, final Runnable callback) {
        new Thread(() -> {
            try {
                ...
                LiteModTwitchIntegration.logger.error("Failed to retrieve your username from the token. Please try again. {}", exceptionMessage);
            }
        }).start();
    }
